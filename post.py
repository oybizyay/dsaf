...
def post():
    ...
    book_new = openpyxl.open("ТК/Отчет Почта  НП от 03.07.23.xlsx")
    ...
    if os.path.exists("пул/p1.DBF"):
        with open("пул/p1.DBF", "r", encoding="CP866") as file:
    ...
    # cod post
    if os.path.exists("пул/p2.xlsx"):
        book = openpyxl.open("пул/p2.xlsx")
        sheet = book.active
    ...
    book_new.save("res/Отчет Почта  НП от %s %s.xlsx" % (yesterday, round(sm, 2)))
    book_new.close()