...
def cdek ():
    ...
    workbook = xlrd.open_workbook("пул/s1.xls", on_demand = True)
    worksheet = workbook.sheet_by_index(0)

    workbook_o = xlrd.open_workbook("пул/s11.xls", on_demand = True)
    worksheet_o = workbook_o.sheet_by_index(0)

    book_new = openpyxl.open("ТК/Отчет СДЭК НП от 25.07.2023.xlsx")
    sheet_new = book_new.active

    if os.path.exists("пул/s2.xls"):

        workbook2 = xlrd.open_workbook("пул/s2.xls", on_demand = True)
        worksheet2 = workbook2.sheet_by_index(0)

        workbook_o2 = xlrd.open_workbook("пул/s22.xls", on_demand = True)
    ...
    ksm = "Контрольные суммы СДЭК: *%s*  *%s*" % (ksm1, ksm2)

    book_new.save("res/Отчет СДЭК НП от %s %s.xlsx" % (yesterday, round(sm, 2)))
    book_new.close()
