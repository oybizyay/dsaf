...
def dpd ():
    ...
    book_new = openpyxl.open("ТК/Отчет ДПД НП от 03.07.23.xlsx")
    sheet_new = book_new.active

    list_np = []
    sm = 0

    book = openpyxl.open("пул/d1.xlsx")
    sheet = book.active
    ...
    if os.path.exists("пул/d2.xlsx"):
        book = openpyxl.open("пул/d2.xlsx")
        sheet = book.active
    ...
    if os.path.exists("пул/d3.xlsx"):
        book = openpyxl.open("пул/d3.xlsx")
        sheet = book.active
    ...
    book_new.save("res/Отчет ДПД НП от %s %s.xlsx" % (yesterday, round(sm, 2)))
    book_new.close()