...
def log_safe():
    ...
    book = openpyxl.open("пул/l1.xlsx")
    sheet = book.active

    if os.path.exists("пул/l2.xlsx"):
        book22 = openpyxl.open("пул/l2.xlsx")
        sheet22 = book22.active

    if os.path.exists("пул/l3.xlsx"):
        book222 = openpyxl.open("пул/l3.xlsx")
        sheet222 = book222.active

    book_new = openpyxl.open("ТК/Отчет Логсис НП СЭЙФ от 03.07.23.xlsx")
    sheet_new = book_new.active

    book_new1 = openpyxl.open("ТК/Отчет Логсис СЧЕТ СЭЙФ 03.07.23.xlsx")
    ...
    for i in range(1, len(lst_sch) + 1):
        ...
        sheet_new1[i + 1][22].value = lst_sch[i - 1][3] * 1.2
    ...
    book_new1.save("res/Отчет Логсис СЧЕТ СЭЙФ от %s.xlsx" % (yesterday))
    book_new.save("res/Отчет Логсис НП СЭЙФ от %s %s.xlsx" % (yesterday, round(sm, 2)))
    ...
    ksm = "Контрольные суммы Логсис: *%s*  *%s*  *%s*" %(ksm1,ksm2,ksm3)
